...
def update_info():
    global tray_icon, time_remaining_tooltip, next_420_info
    ...
    if total_seconds >= 0:
        ...
        # Pause updates for 15 seconds to avoid spamming
        root.after(15000, get_next_420)
        root.after(16000, update_info)
...
def open_settings_window():
    ...
    tk.Label(settings_window, text="Notification Text:", bg=dark_bg, fg=light_fg).pack(pady=5)
    notif_text_var = tk.StringVar(value=settings.get("notification_text", "It's 4:20 somewhere!"))
    notif_entry = tk.Entry(settings_window, textvariable=notif_text_var, width=40, bg="#444444", fg=light_fg)
    notif_entry.pack()

    run_on_startup_var = tk.BooleanVar(value=settings.get("run_on_startup", False))
    run_on_startup_check = tk.Checkbutton(settings_window, text="Run on System Startup",
                                          variable=run_on_startup_var, bg=dark_bg, fg=light_fg,
                                          selectcolor=dark_bg)
    run_on_startup_check.pack(pady=5)

    tk.Label(settings_window, text="Note: Enabling startup may cause\nWindows to block the app.",
             fg="red", bg=dark_bg).pack(pady=5)
    ...
    tk.Button(settings_window, text="Save", command=save_and_close,
              bg=button_bg, fg=button_fg, activebackground="#444444", activeforeground=light_fg).pack(pady=10)
...
label = tk.Label(root, textvariable=label_var, wraplength=300, justify="left", bg=dark_bg, fg=light_fg)
label.pack(pady=10)
buttons_frame = tk.Frame(root, bg=dark_bg)
buttons_frame.pack(pady=10)
toggle_button_var = tk.StringVar(value=f"Minimize to Tray: {'Enabled' if settings['minimize_to_tray'] else 'Disabled'}")
toggle_button = tk.Button(buttons_frame, textvariable=toggle_button_var, command=toggle_minimize_to_tray, bg=button_bg, fg=button_fg)
toggle_button.grid(row=0, column=0, padx=5)
settings_button = tk.Button(buttons_frame, text="Settings", command=open_settings_window, bg=button_bg, fg=button_fg)
